# Claude Code Project Instructions — dns-lookup-kotlin

## Project Overview

This is **dns-lookup-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`dns-lookup`](https://crates.io/crates/dns-lookup).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/dns-lookup/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/dns-lookup>
