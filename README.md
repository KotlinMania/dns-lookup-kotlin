...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`dns-lookup`](https://crates.io/crates/dns-lookup).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `dns-lookup` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `dns-lookup` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
